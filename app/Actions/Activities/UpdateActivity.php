...
class UpdateActivity
{
        ...
        return $this->executeInTenantContext($tenantId, function () use ($slug, $data) {
            ...
            if ($shouldSyncImages) {
                if (isset($data['_images_cleared']) && $data['_images_cleared'] === '1') {
                    ...
                    // Get media to delete
                    $mediaToDelete = $updatedActivity->getMedia('images')
                ...
                if (isset($data['uploaded_media_ids']) && is_array($data['uploaded_media_ids']) && !empty($data['uploaded_media_ids'])) {
                    ...
                    foreach ($data['uploaded_media_ids'] as $index => $tempUploadId) {
                        ...
                        if ($tempUpload && $tempUpload->media->isNotEmpty()) {
                            ...
                            $newMedia->model_type = get_class($updatedActivity);
                            $newMedia->model_id = $updatedActivity->id;
    ...
    /**
     * Sync features for an activity with categories from the Features management page.
     *
     * @param string $tenantId
     * @param string $slug
    ...
    public function syncFeatures(string $tenantId, string $slug, array $featuresData): void
    {
        $this->executeInTenantContext($tenantId, function () use ($slug, $featuresData) {
            ...
            $activity->features()->sync($syncData);
            $activity->touch();
        });
    }
}
