...
const commissionType = ref<string>(props.activity.commission_config?.commission_type || 'percentage');
const commissionPercentage = ref<number>(props.activity.commission_config?.commission_percentage || 5);
const commissionFixed = ref<number>(props.activity.commission_config?.commission_fixed || 0);
const currency = ref<string>(props.activity.commission_config?.currency || 'IDR');
...
<template>
    <BaseFormPage
        ...
        :action="activities.commission.update.url([activity.tenant_id, activity.slug])"
        method="post"
