...
<template>
    <TenantLayout v-slot="{ tenant }" :title="'Treatments'">
        <div class="min-h-dvh p-8">
            <div class="max-w-6xl mx-auto">
                <h1 class="text-3xl font-bold mb-2">Spa Treatments</h1>
                <p class="text-gray-600 dark:text-gray-400 mb-8">
                    Rejuvenating treatments at {{ tenant.name }}
                </p>
                    ...
                    >
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm mb-4">
                                Experience our {{ item.name.toLowerCase() }}
                            </p>
                            <Link 
                                :href="nested({ parent: resourceSlug, child: item.slug })"
                                class="inline-block px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg text-sm transition-colors"
                            >
                                Book Now →
                            </Link>
                        </div>
                ...
                <div v-if="resources.length === 0" class="text-center py-16">
                    <p class="text-gray-500">No treatments available at this time.</p>
                </div>
            </div>
        </div>
    </TenantLayout>
