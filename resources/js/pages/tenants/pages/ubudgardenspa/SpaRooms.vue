...
<template>
    <TenantLayout v-slot="{ tenant }" :title="'Spa Rooms'">
        <div class="min-h-dvh p-8">
            <div class="max-w-6xl mx-auto">
                <h1 class="text-3xl font-bold mb-2">Our Spa Rooms</h1>
                <p class="text-gray-600 dark:text-gray-400 mb-8">
                    Tranquil treatment spaces at {{ tenant.name }}
                </p>
                    ...
                    >
                        ...
                        <div class="p-6">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm mb-4">
                                Relax in our {{ item.name.toLowerCase() }}
                            </p>
                            <Link 
                                :href="nested({ parent: resourceSlug, child: item.slug })"
                                class="inline-block px-4 py-2 bg-violet-500 hover:bg-violet-600 text-white rounded-lg text-sm transition-colors"
                            >
                                View Details →
                            </Link>
                        </div>
                ...
                <div v-if="resources.length === 0" class="text-center py-16">
                    <p class="text-gray-500">No spa rooms available at this time.</p>
                </div>
            </div>
        </div>
    </TenantLayout>
