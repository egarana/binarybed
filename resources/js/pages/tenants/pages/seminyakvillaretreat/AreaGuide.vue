...
<template>
    <TenantLayout v-slot="{ tenant }" :title="'Area Guide'">
        <div class="flex flex-col items-center justify-center min-h-dvh p-8">
            <h1 class="text-3xl font-bold mb-4">Area Guide</h1>
            <h2 class="text-xl font-medium">{{ tenant.name }}</h2>
            ...
            <nav class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl">
               ...
               <Link :href="tenantRoutes.contact()" class="px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-center transition-colors">Contact Us</Link>
                ...
                <Link :href="tenantRoutes.page('villas')" class="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg text-center transition-colors">Villas</Link>
                <Link :href="tenantRoutes.page('experiences')" class="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg text-center transition-colors">Experiences</Link>
            </nav>
        </div>
    </TenantLayout>
</template>
